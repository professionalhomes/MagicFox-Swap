...
async function main() {
  ...
  const BLUECHIP_VOTER = await hre.ethers.getContractAt('BluechipVoter', addresses.bluechipVoter, deployer);
    ...
    let tx;
    tx = await VOTER.setSidechainManager(constants.ARBITRUM.lzChainId, constants.ARBITRUM.lzReceiver);
    await tx.wait();
    console.log("VOTER.setSidechainManager");

    tx = await BLUECHIP_VOTER.setSidechainManager(constants.ARBITRUM.lzChainId, constants.ARBITRUM.lzReceiverBluechip);
    await tx.wait();
